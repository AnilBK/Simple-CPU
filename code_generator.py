...
def set_jump_if_last_op_small(new_program_counter):
    _code = ""
    _code += (
        str(OPCODE_JUMP_TO_IF_LAST_OP_SMALL)
        + " , "
        + str(new_program_counter).strip()
        + "\n"
    )
    return _code
...
def set_jump_if_last_op_greater(new_program_counter):
    _code = ""
    _code += (
        str(OPCODE_JUMP_TO_IF_LAST_OP_GREATER)
        + " , "
        + str(new_program_counter).strip()
        + "\n"
    )
...
def set_jump_if_last_op_equal(new_program_counter):
    _code = ""
    _code += (
        str(OPCODE_JUMP_TO_IF_LAST_OP_EQUAL)
        + " , "
        + str(new_program_counter).strip()
        + "\n"
    )
...
def set_Memory_Address(value):
    param = get_combined_output(
        0, 0, REGISTER_MEMORY_ADDRESS, 0, 1
    )  # Registers reads from input line.
...
def set_Memory_Address_equals_tmp():
    param = get_combined_output(
        1, 0, REGISTER_MEMORY_ADDRESS, 0, 1
    )  # Registers reads from input line.
